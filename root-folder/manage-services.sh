...
# Discovers compose files in /home/chris/apps and /home/chris/github/hephaestus-homelab
# Provides commands: up, down, restart, ps, logs, pull, build

ROOT_DIR="/home/chris"
APPS_DIR="$ROOT_DIR/apps"
INFRA_DIR="$ROOT_DIR/github/hephaestus-homelab"
...
discover_compose_files() {
	local found=()
	if [ -d "$INFRA_DIR" ]; then
		while IFS= read -r -d '' file; do found+=("$file"); done < <(find "$INFRA_DIR" -type f \( -name 'docker-compose-homelab.yml' -o -name 'docker-compose.yml' -o -name 'docker-compose*.yml' \) -print0 2>/dev/null)
	fi
	if [ -d "$APPS_DIR" ]; then
...
map_category() {
	...
	if [[ "$lc" == *"github/hephaestus-homelab"* || "$lc" == *"proxy"* ]]; then echo infra; return; fi
